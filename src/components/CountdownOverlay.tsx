...
export default function CountdownOverlay({ onUnlock }: Props) {
  ...
  useEffect(() => {
    if (!celebrating || playedMinionsRef.current) return;
    playedMinionsRef.current = true;
    playMinions();
  }, [celebrating, playMinions]);
        ...
        {celebrating ? (
          <div className="flex flex-col items-center gap-5 sm:gap-7">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={playMinions}
                title="Програти привітання ще раз"
                className="flex items-center justify-center w-12 h-12 shrink-0
                                           bg-white text-primary rounded-full shadow-lg border border-primary/20
